...
由三元二次方程确定的曲面统称为 **二次曲面**，如球面，圆柱面，旋转抛物面等。判断曲面形状的方法为 **截痕法**，即用平行于坐标面的平面（截面）取截曲面，根据截痕的变化判断曲面的形状。
...
由方程 $\frac{x^2}{a^2}+\frac{y^2}{b^2}-\frac{z^2}{c^2}=1（其中，a>0,b>0,c>0）$ 表示的曲面为 **单叶双曲面**。
