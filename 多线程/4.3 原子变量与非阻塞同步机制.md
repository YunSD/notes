...
在实际的实现中，ConcurrentLinkedQueue 中没有使用原子引用来表示每个 Node，而是使用普通的 volatile 类型引用，并通过基于反射的  AtomicReferenceFieldUpdater 来进行更新，在 Java 8 中，该类的实现实际使用了 Unsafe 类来进行原子操作。在 Java 8 的包中仅仅有三个类使用到这个原子域更新器类（BufferedInputStream（jdk1.0）、SelectionKey（jdk1.4）、SQLException（jdk版本很早的类））。新的jdk版本的类都不在使用该类了。而是直接使用 Unsafe 类。
